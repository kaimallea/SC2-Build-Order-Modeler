//
// Internet Explorer doesn't have a built-in
    ...
    if (this.build_order.length) {
        ...
        for (var i=0; i < this.build_order.length; i++) {
            ...
            if (item) {
                ...
                // TODO: Fix check so units don't get cut off 
                // after 10 (the zero)
                
                // Did we just add this?
                if (item.uid == url.substr(-5, 3)) { // Yes we did
                    // Get last consecutive count
                    last_amount = parseInt(url.substr(-2, 1));
                    // Increment consecutive count
                    last_amount += 1;
                    // Re-assign string with new value + delimiter
...
//
// Re-create build order from url
//
BuildOrder.prototype.build_from_url = function (url) {
    if (!url) {
        err("Please provide a valid url");
    }
    
    // TODO: Break down url and parse after # or ?
    ...
    uidloop: for (var i=0; i < uids.length; i++) {
        ...
        for (var x in this.units) {
            ...
            if (this.units[x].uid == uids[i].substr(0, 3)) {
                ...
                for (var n=0; n < parseInt(uids[i].substr(-1)); n++) {
                    this.build_order.push(x);
                }
...
BuildOrder.prototype.train = function(unit) {
    ...
    if (u) {
        ...
        if (parseInt(u.supply, 10) > limit) {
            err("build: You need more supply!");
        }

        // Check if unit's build_from structure exists
        if (this.build_order.indexOf(u.build_from) >= 0) {
        ...
        if (prerequisite) {
            if (is_array(prerequisite)) {   // Multiple prereqs (array)
                for (var i = 0; i < prerequisite.length; i++) {
                    ...
                    if (this.build_order.indexOf(prerequisite[i]) == -1) {
                        // One is missing!
            ...
            {
                // Check for prereq
                if (this.build_order.indexOf(prerequisite) == -1) {
                    // Nope, not built!
...
//
Protoss.prototype = new BuildOrder();
...
// Inherits from BuildOrder
//
Terran.prototype = new BuildOrder();
...
/*
var a = new Terran();
var b = a.generate_url();
a.build_order = [];
print(a.build_order);
var c = a.build_from_url(b);
print(a.build_order);
*/

/*
for (var a in b.upgrades) {
    print(a);
}
// Tests
try
{
    a = new Terran();
    print(a.total_cost.supply.used + "/" + a.total_cost.supply.total);
    a.build("SCV");
    a.build("SCV");
    a.build("SCV");
    a.build("SCV"); // 10
    print(a.total_cost.supply.used + "/" + a.total_cost.supply.total);
    a.construct("Barracks");
    print(a.total_cost.supply.used + "/" + a.total_cost.supply.total);
    a.construct("Tech Lab");
    a.upgrade("Stimpack");
    a.build("Marine"); // 11th supply
    print(a.build_order);
    a.build("SCV"); // Need more supply!
}
catch(e)
{
    print(e.message);
