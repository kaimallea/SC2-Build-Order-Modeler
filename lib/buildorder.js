...
BuildOrder.prototype.train = function(unit) {
    ...
    if (u) {
        ...
        // Prevent building anymore units if over supply max reached
        if (this.total_cost.supply.used == 200) {
            err("Supply maximum (200) cannot be exceeded");
        }
        
        // Prevent building anymore units if supply will be exceeded
        var f = this.total_cost.supply.used + parseInt(u.supply, 10);
        if (f > 200) {
            err("Supply maximum will be breached");
        }
        ...
        if (this.default_build_order.indexOf(u.build_from) >= 0 || 
            ...
            // Prerequisites / Add-ons met?
            if (u.prerequisite) {
                // More than one?
                if (is_array(u.prerequisite)) {
                    // Check for them all
                    for (var i = 0; i < u.prerequisite.length; i += 1) {
                        if (this.build_order.indexOf(u.prerequisite[i] == -1)) {
                            err("train: " + unit + " requires " + u.prerequisite);
                        }
                    }
                // Only one prereq
                } else {
                    if (this.build_order.indexOf(u.prerequisite) == -1) {
                        err("train: " + unit + " requires " + u.prerequisite);
                    }
                }
            }
...
BuildOrder.prototype.construct = function(struct) {
    ...
    if (s) {
        ...
        // Special case: Zerg construction reduces supply count by 1
        if (this.race == "zerg") {
        ...
        if (prerequisite) {
            ...
            else // Single prereq
            {
                ...
                if (this.default_build_order.indexOf(prerequisite) == -1 &&
                    this.build_order.indexOf(prerequisite) == -1) {
                    // Nope, not built!
                    err("upgrade: " + upgrade + " requires " + prerequisite);
                }
            }
        }
