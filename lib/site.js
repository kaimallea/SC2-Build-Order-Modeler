...
function render(what) {
    ...
    var trim = what.split(' ').join('').toLowerCase();
    
    $("<div class='unit'>" + current_supply + " | " + what + "</div>")
        .attr("style", "background-image: url('img/units/" + trim + ".jpg'")
        .hide()
...
var bitly = {
    ...
    callback: function (response) {
        if (response.status_code == 200) {
            $("#sharable-url")[0].value = response.data.url; 
        } else {
...
$("#shorten-url").click(function () {
    try 
    {
        bitly.shorten($("#sharable-url")[0].value);
    } 
    catch(e)
    {
        alert(e);
